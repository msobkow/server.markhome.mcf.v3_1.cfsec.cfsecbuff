// Description: Java 25 implementation of a ServiceType buffer
...
public class CFSecBuffServiceType
	implements ICFSecServiceType, Comparable<Object>, Serializable
{
	...
	public List<ICFSecService> getOptionalChildrenDeployed() {
		...
		if (targetBackingSchema == null) {
			throw new CFLibNullArgumentException(getClass(), "setOptionalChildrenDeployed", 0, "ICFSecSchema.getBackingCFSec()");
		}
		ICFSecServiceTable targetTable = targetBackingSchema.getTableService();
		if (targetTable == null) {
			throw new CFLibNullArgumentException(getClass(), "setOptionalChildrenDeployed", 0, "ICFSecSchema.getBackingCFSec().getTableService()");
		}
		ICFSecService[] targetArr = targetTable.readDerivedByTypeIdx(null, getRequiredServiceTypeId());
		if( targetArr != null ) {
			List<ICFSecService> results = new ArrayList<>(targetArr.length);
			...
			List<ICFSecService> results = new ArrayList<>();
			return( results );
		}
	}
	@Override
	...
	public String toString() {
		String ret = "<CFSecBuffServiceType" + getXmlAttrFragment() + "/>";
		return( ret );
	}
}
